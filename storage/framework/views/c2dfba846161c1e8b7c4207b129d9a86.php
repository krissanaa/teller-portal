...
<div class="container-fluid">
    ...
    <div class="card shadow-sm mb-3">
        <div class="card-body">
            <div class="d-flex align-items-center justify-content-between mb-2">
                <?php if(auth()->user()->isBranchAdmin() || auth()->user()->isAdmin()): ?>
                <h5 class="mb-0 text-primary">
                    <i class="bi bi-person-badge me-1"></i>
                    <?php echo e($request->teller->teller_id ?? $request->teller_id); ?> - <?php echo e($request->teller->name ?? 'Unknown'); ?>
            ...
            <div class="row">
                <div class="col-md-6">
                    <p><strong>ຊື່ຮ້ານ:</strong> <?php echo e($request->store_name); ?></p>
                    <p><strong>ປະເພດທຸລະກິດ:</strong> <?php echo e($request->business_type); ?></p>
                    <p><strong>ເລກບັນຊີທະນາຄານ:</strong> <?php echo e($request->bank_account ?? '-'); ?></p>
                    <p><strong>ວັນທີຕິດຕັ້ງ:</strong> <?php echo e(\Carbon\Carbon::parse($request->installation_date)->format('d/m/Y')); ?></p>
                ...
                <div class="col-md-6">
                    <p><strong>ທີ່ຢູ່ຮ້ານ:</strong> <?php echo e($request->store_address); ?></p>
                    <p><strong>ລະຫັດຜູ້ແນະນຳ:</strong> <?php echo e($request->refer_code); ?></p>
                    <p><strong>ເລກຊີຣີ POS:</strong> <?php echo e($request->pos_serial ?? '-'); ?></p>
                    <p><strong>ວັນທີສ້າງ:</strong> <?php echo e($request->created_at->format('d/m/Y H:i')); ?></p>
                </div>
            </div>
            ...
            <div class="d-flex flex-wrap gap-3">
                ...
                ?>
                <?php $publicUrl = asset('storage/' . ltrim($filePath, '/')); ?>
                <div class="att-card" onclick="openPreview('<?php echo e(route('storage.file', ['path' => $filePath])); ?>', '<?php echo e($fileName); ?>', '<?php echo e($extension); ?>', '<?php echo e(asset('storage/' . $filePath)); ?>')">
                    <div class="att-preview">
                        <?php if(in_array($extension, ['jpg','jpeg','png'])): ?>
                        <img src="<?php echo e(route('storage.file', ['path' => $filePath])); ?>"
                            class="rounded"
                            data-fallback-url="<?php echo e(asset('storage/' . $filePath)); ?>"
                            alt="<?php echo e($fileName); ?>"
                            onerror="this.src=this.dataset.fallbackUrl; this.onerror=null;">
                        <?php elseif($extension === 'pdf'): ?>
                        <canvas class="rounded" data-pdf-thumb data-url="<?php echo e(route('storage.file', ['path' => $filePath])); ?>" style="background:#f8fafc;"></canvas>
                        <?php else: ?>
                        <i class="bi bi-file-earmark text-secondary fs-1"></i>
                        <?php endif; ?>
                    </div>
                    <div class="att-name" title="<?php echo e($fileName); ?>"><?php echo e($fileName); ?></div>
                    <div class="att-meta">
                        <span class="text-muted small">Preview</span>
                        <span class="att-ext"><?php echo e(strtoupper($extension)); ?></span>
                    </div>
                </div>
            ...
            <div class="alert alert-danger mt-3">
                <strong>ໝາຍເຫດຈາກຜູ້ດູແລ:</strong> <?php echo e($request->admin_remark); ?>
    ...
    <div class="d-flex gap-2">
        <a href="<?php echo e(route('teller.requests.edit', $request->id)); ?>" class="btn btn-warning">ແກ້ໄຂຄຳຂໍ</a>
        <a href="<?php echo e(route('teller.dashboard')); ?>" class="btn btn-secondary">ກັບຄືນໜ້າຫຼັກ</a>
    </div>
</div>
